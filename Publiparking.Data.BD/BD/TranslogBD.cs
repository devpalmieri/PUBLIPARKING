...
namespace Publiparking.Data.BD
{
    public class TranslogBD : EntityBD<translog>
    {
        ...
        /// <returns></returns>
        public static IQueryable<TransLogDTO> getListTranslogLocalTargheByTarga(string p_targa,DbParkCtx p_context)
        {
            
            IQueryable<TransLogDTO> v_listTransLog = GetList(p_context).Where(t => t.tlLicenseNo_isNumeric.HasValue && t.tlLicenseNo_isNumeric.Value == 0)
            ...
            IQueryable<TransLogDTO> v_listpyng = TranslogPyngBD.GetList(p_context).Where(t => t.tlExpDateTime.HasValue && t.tlPayDateTime.HasValue)
                                                                                  .Where(t => t.tlLicenseNo.Trim() == p_targa)
                                                                                  .OrderByDescending(o => o.tlPayDateTime.Value)
                                                                                  .Take(1)
                                                                                  .ToTranslogDTO();

            IQueryable<TransLogDTO> v_listphonzie = TranslogPhonzieBD.GetList(p_context).Where(t => t.tlExpDateTime.HasValue && t.tlPayDateTime.HasValue)
                                                                                       .Where(t => t.tlLicenseNo.Trim() == p_targa)
                                                                                       .OrderByDescending(o => o.tlPayDateTime.Value)
                                                                                       .Take(1)
                                                                                       .ToTranslogDTO();            
        ...
        public static decimal getTotaleRicariche(string p_codice_abbonamento, DbParkCtx p_context)
        {
            return GetList(p_context).Where(a => a.tlLicenseNo.Equals(p_codice_abbonamento)).Sum(a => a.tlAmount) ?? 0;
        }

        public static IQueryable<translog> getPagamentoByCodiceAbbonamento(string p_codice_abbonamento, DbParkCtx p_context)
        {
            return GetList(p_context).Where(a => a.tlLicenseNo.Equals(p_codice_abbonamento));
        }

        public static IQueryable<translog> getListRicaricheNonConfermate(DbParkCtx p_context)
        {
            // List<int> rcIdsList = RicaricheConfermateBD.GetList(p_context).Select(rc=> rc.idRicaricaConfermata).ToList();
            List<string> codiceabbonamentiList = AbbonamentiBD.GetList(p_context).Select(rc => rc.codice).Distinct().ToList();

            return GetList(p_context).Where(r => !string.IsNullOrEmpty(r.tlLicenseNo) && r.tlLicenseNo.Length == 12)
        ...
        public static IQueryable<translog> getListRicaricheAbbonamentiDaElaborare(DbParkCtx p_context)
        {
            List<string> codiceabbonamentiPeriodiciList = AbbonamentiPeriodiciBD.GetList(p_context).Select(rc => rc.codice).Distinct().ToList();
            List<string> codiceabbonamentiRinnoviList = AbbonamentiRinnoviBD.GetList(p_context).Select(rc => rc.codice).Distinct().ToList();

            return getListRicaricheAbbonamenti(p_context).Where(r => codiceabbonamentiPeriodiciList.Contains(r.tlLicenseNo))
                                                         .Where(r => !codiceabbonamentiRinnoviList.Contains(r.tlPDM_TicketNo_union));
        }

        public static IQueryable<translog> getListRicaricheAbbonamenti(DbParkCtx p_context)
        {
            return GetList(p_context).Where(r => !string.IsNullOrEmpty(r.tlLicenseNo) && r.tlLicenseNo.Length == 12)
                                     .Where(r => r.tlPayType.HasValue && r.tlPayType.Value > 0)
                                     .Where(r => r.tlAmount.HasValue && r.tlAmount.Value > 0)
        ...
        public static void confermaRicaricaAbbonamento(translog p_ricarica, string p_numeroMittente, DbParkCtx p_context)
        {

            SMSOut v_SMSOut = new SMSOut();
            string v_codiceAbbonamento = p_ricarica.tlLicenseNo;

            Abbonamenti v_abbonamento = AbbonamentiBD.getAbbonamentoByCodice(v_codiceAbbonamento, p_context);

            if (v_abbonamento != null)
            {
                Cellulari v_cellulare = CellulariBD.getCellulareByIdAbbonamentoAndMaster(v_abbonamento.idAbbonamento, p_context);
                ...
                if (v_cellulare != null)
                {
                    ...
                    p_context.RicaricheConfermate.Add(v_ricaricaConfermata);
