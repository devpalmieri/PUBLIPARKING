...
namespace Publiparking.Data.BD
{
    public class AbbonamentiRinnoviBD : EntityBD<AbbonamentiRinnovi>
    {
        ...
        public static bool rinnova(int p_idAbbonamento, Int64 p_idRicarica, DbParkCtx p_parkcontext)
        {
            bool risp = false;
            translog v_ricarica = TranslogBD.GetById(p_idRicarica, p_parkcontext);
            AbbonamentiPeriodici v_abbonamento = AbbonamentiPeriodiciBD.GetById(p_idAbbonamento, p_parkcontext);
            if (v_ricarica != null & v_abbonamento != null)
            {
                ...
                AbbonamentiRinnovi v_ultimoRinnovo = AbbonamentiRinnoviBD.GetList(p_parkcontext)
                ...
                TariffeAbbonamenti v_tariffa = TariffeAbbonamentiBD.GetById(v_abbonamento.idTariffaAbbonamento, p_parkcontext);
                bool v_fasciaTrovata = false;

                IQueryable<FasceTariffeAbbonamenti> v_fasciePerAbbonamento = FasceTariffeAbbonamentiBD.GetList(p_parkcontext).Where(f => f.idTariffaAbbonamento == v_tariffa.idTariffaAbbonamento);
                ...
                if (v_fasciaTrovata)
                {
                    p_parkcontext.AbbonamentiRinnovi.Add(v_rinnovo); 
                }
